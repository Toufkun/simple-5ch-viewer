...
// ルート: /        …トップ
//       /menus    …カテゴリタイル一覧
//       /menus/c  …カテゴリ内の板一覧 (?cat=)
//       /boards   …板一覧（テキスト検索あり）
//       /board    …スレ一覧 (?url=板URL)
//       /thread   …本文表示 (?base=板URL&dat=番号) or (?url=read.cgi/dat URL)
//       /__diag   …診断API
//
// 環境変数:
//   PROXY_URL       (必須) Cloudflare Worker のURL (例: https://xxxx.workers.dev)
//   BASE_BOARD_URL  (任意) 既定の板URL (例: https://mi.5ch.net/news4vip/)
//   BBSMENU_URL     (任意) メニューURL固定 (未設定なら候補を順に試行)
...
const DEFAULT_BASE = (process.env.BASE_BOARD_URL || '').trim();            // https://mi.5ch.net/news4vip/
const PROXY_URL    = (process.env.PROXY_URL || '').replace(/\/+$/, '');    // https://xxxx.workers.dev
...
const THEME_STYLE = `
<style>
  :root { --bg:#fff; --fg:#111; --muted:#666; --link:#0a58ff; --card:#f7f7f8; --border:#e5e7eb; }
  ...
  body{ font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial; line-height:1.6;
        padding:16px; max-width:940px; margin:auto; }
  ...
  pre{ background:var(--card); border:1px solid var(--border); border-radius:8px; padding:10px 12px;
       white-space:pre-wrap; word-break:break-word; }
  .theme-toggle{ position:fixed; top:12px; right:12px; cursor:pointer; font-size:18px;
...
(function(){
  ...
  function setIcon(){
    ...
    const d=document.documentElement.dataset.theme==="dark"; b.textContent=d?"☀️":"🌙";
    b.setAttribute("aria-label", d?"ライトに切替":"ダークに切替");
  }
  function toggle(){ const cur=document.documentElement.dataset.theme==="dark"?"light":"dark";
    document.documentElement.dataset.theme=cur; localStorage.setItem("theme",cur); setIcon(); }
  addEventListener('DOMContentLoaded',()=>{ const b=document.createElement('button');
    b.id='theme-toggle'; b.className='theme-toggle'; b.onclick=toggle; document.body.appendChild(b); setIcon(); });
})();
</script>`;
...
function anchorizeEscapedText(txt) {
  // he.escape 済みテキスト内の >>n を内部リンクに
  return txt.replace(/&gt;&gt;(\d+)/g, '<a class="anc" href="#r$1">&gt;&gt;$1</a>');
}

/* 文字コード推定 & デコード（read.cgi向け） */
function sniffCharsetFromHeaders(h={}) {
  const ct=(h['content-type']||h['Content-Type']||'').toLowerCase(); const m=ct.match(/charset\s*=\s*([^;]+)/);
  return m?m[1].trim():'';
}
function sniffCharsetFromHtmlHead(buf) {
  const head=Buffer.from(buf).slice(0,4096).toString('ascii');
  const m=head.match(/charset\s*=\s*["']?\s*([a-zA-Z0-9_\-]+)/i); return m?m[1].toLowerCase():'';
}
...
function decodeHtmlBinary(binary, headers) {
  const cs = normalizeCharset(sniffCharsetFromHeaders(headers)) ||
             normalizeCharset(sniffCharsetFromHtmlHead(binary)) || 'cp932';
...
function parseReadCgiHtml(html) {
  ...
  $('article, .post, .postWrap, .postContainer, li.post, .res, .reply').each((i, el) => {
    const name = (
      $(el).find('.name').text() || $(el).find('.name a').text() ||
      $(el).find('.username').text() || $(el).find('.poster').text() || ''
    ).trim();
    const dateId = (
      $(el).find('.date').text() || $(el).find('.info').text() ||
      $(el).find('.meta').text() || ''
    ).trim();
    const bodyHtml =
      $(el).find('.message, .post-message, .body, .messageText, .content, .message .text').html() ||
      $(el).find('blockquote').html() ||
      $(el).find('.mes, .msg, .resbody').html() || $(el).html();
    if (bodyHtml) {
...
app.get('/board', async (req, res) => {
  try {
    ...
    const r = await getVia(subjectUrl, { binary:true });
...
${THEME_SCRIPT}`);
    ...
    if (rHtml.status !== 200) return res.status(rHtml.status).send('取得に失敗しました: READCGI_' + rHtml.status);
    ...
    res.send(`<!doctype html><meta charset="utf-8"><title>本文(read.cgi)</title>
${THEME_STYLE}
<p><a href="/board?url=${encodeURIComponent(base)}">← スレ一覧へ戻る</a></p>
${body || 'レスがありません'}
${THEME_SCRIPT}`);
...
<style>
  ...
  .tile{display:flex;align-items:center;gap:10px;background:var(--card);border:1px solid var(--border);
        border-radius:12px;padding:14px 16px;color:var(--fg);text-decoration:none}
...
app.get('/menus/c', async (req, res) => {
  try {
    ...
    const list = hit.boards.map(b => `
      <div class="row">
        <a href="/board?url=${encodeURIComponent(b.url)}">${he.escape(b.name)}</a>
        <span class="muted url">${he.escape(b.url)}</span>
      </div>`).join('');
...
<style>
  ...
  .row{display:flex;flex-direction:column;padding:8px 6px;border-bottom:1px solid var(--border)}
...
app.get('/__diag', async (req, res) => {
  try {
    ...
    res.json({ proxy_url: PROXY_URL || null, subject_status:s, dat_status:d, readcgi_status:r });
  } catch (e) {
